...
func GetEmbedEventsPartial(w http.ResponseWriter, r *http.Request) http.HandlerFunc {
	...
	// Helper function to get query param with mnm_ prefix fallback
	getEmbedParam := func(key string) string {
		// First check for mnm_ prefixed param
		if val := r.URL.Query().Get("mnm_" + key); val != "" {
			return val
		}
		// Fall back to regular param
		return r.URL.Query().Get(key)
	}

	// Get ownerId from query parameters
	ownerId := getEmbedParam("ownerId")
	if ownerId == "" {
		// Also check without prefix for backward compatibility
		ownerId = r.URL.Query().Get("ownerId")
		if ownerId == "" {
			return transport.SendHtmlRes(w, []byte("Missing required parameter: ownerId"), http.StatusBadRequest, "partial", errors.New("missing ownerId"))
		}
	}

	// Check if user has subGrowth role
	userRoles, err := helpers.GetUserRoles(ownerId)
	if err != nil {
		log.Printf("Error fetching user roles for ownerId %s: %v", ownerId, err)
		return transport.SendHtmlRes(w, []byte("Failed to verify user permissions"), http.StatusInternalServerError, "partial", err)
	}

	// Convert string roles to RoleClaim slice for HasRequiredRole
	roleClaims := make([]constants.RoleClaim, 0, len(userRoles))
	for _, role := range userRoles {
		roleClaims = append(roleClaims, constants.RoleClaim{Role: role})
	}

	// Check if user has subGrowth role
	if !helpers.HasRequiredRole(roleClaims, []string{constants.Roles[constants.SubGrowth]}) {
		return transport.SendHtmlRes(w, []byte("User does not have permission to embed events"), http.StatusNotFound, "partial", errors.New("user lacks subGrowth role"))
	}

	// Get user data for pageUser
	pageUser, err := helpers.GetOtherUserByID(ownerId)
	if err != nil {
		log.Printf("Error fetching user data for ownerId %s: %v", ownerId, err)
		return transport.SendHtmlRes(w, []byte("Failed to fetch user data"), http.StatusInternalServerError, "partial", err)
	}

	// Get about metadata
	aboutData, _ := helpers.GetOtherUserMetaByID(ownerId, constants.META_ABOUT_KEY)
	...
	pageUser.UserID = ownerId

	// Extract search parameters - create a modified request with stripped mnm_ prefixes
	// We need to create a new request with params that GetSearchParamsFromReq expects
	modifiedReq := r.Clone(ctx)
	modifiedReq.URL.RawQuery = ""
	q := modifiedReq.URL.Query()
	
	// Copy params, stripping mnm_ prefix
	...
	for embedKey, apiKey := range paramMap {
		if val := getEmbedParam(embedKey); val != "" {
			q.Set(apiKey, val)
		}
	}
	modifiedReq.URL.RawQuery = q.Encode()
	
	// Now use GetSearchParamsFromReq with the modified request
	searchQ, userLocation, radius, startTimeUnix, endTimeUnix, cfLocation, _, categories, address, parseDates, eventSourceTypes, eventSourceIds := GetSearchParamsFromReq(modifiedReq)

	// Override ownerIds to only include the embed owner
	...
	res, err := services.SearchWeaviateEvents(ctx, weaviateClient, searchQ, userLocation, radius, startTimeUnix, endTimeUnix, ownerIds, categories, address, parseDates, eventSourceTypes, eventSourceIds)
	if err != nil {
		return transport.SendServerRes(w, []byte("Failed to get events via search: "+err.Error()), http.StatusInternalServerError, err)
	}
	...
	// Get search style (default: "inline")
	searchStyle := getEmbedParam("searchStyle")
	if searchStyle == "" {
		searchStyle = r.URL.Query().Get("searchStyle")
		if searchStyle == "" {
			searchStyle = "inline"
		}
	}
	...
	latStr := getEmbedParam("lat")
	lonStr := getEmbedParam("lon")
	if latStr == "" && len(userLocation) >= 2 {
		latStr = strconv.FormatFloat(userLocation[0], 'f', -1, 64)
	}
	if lonStr == "" && len(userLocation) >= 2 {
		lonStr = strconv.FormatFloat(userLocation[1], 'f', -1, 64)
	}
	origLat := getEmbedParam("lat")
	origLong := getEmbedParam("lon")
	origQueryLocation := getEmbedParam("location")

	// Get API base URL from environment
	apiBaseUrl := os.Getenv("APEX_URL")
	if apiBaseUrl == "" {
		apiBaseUrl = "https://meetnearme.app"
	}

	// Render EventsInner separately to avoid import cycle
	eventsInner := pages.EventsInner(events, constants.EV_MODE_LIST, roleClaims, ownerId, &pageUser, true, apiBaseUrl)
	var eventsBuf bytes.Buffer
	err = eventsInner.Render(ctx, &eventsBuf)
	if err != nil {
		return transport.SendHtmlRes(w, []byte(err.Error()), http.StatusInternalServerError, "partial", err)
	}
	eventsHTML := eventsBuf.String()

	// Render embed template
	embedPartial := partials.EmbedEvents(eventsHTML, &pageUser, cfLocation, latStr, lonStr, origLat, origLong, origQueryLocation, searchStyle, roleClaims, ownerId, apiBaseUrl)

	var buf bytes.Buffer
	err = embedPartial.Render(ctx, &buf)
	if err != nil {
		return transport.SendHtmlRes(w, []byte(err.Error()), http.StatusInternalServerError, "partial", err)
	}
