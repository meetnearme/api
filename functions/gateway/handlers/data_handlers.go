...
func SimpleHelloWorldHandler(w http.ResponseWriter, r *http.Request) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = json.Unmarshal(body, &messageJson)
		if err != nil {
			transport.SendServerRes(w, []byte("Invalid JSON payload: "+err.Error()), http.StatusUnprocessableEntity, err)
			return
		}

		message, ok := messageJson["message"]
		if !ok {
			transport.SendServerRes(w, []byte("Request body must include a 'message' field"), http.StatusBadRequest, nil)
			return
		}
		...
		log.Println("simple hello world successfully connected to weaviate.")

		testClassName := "ConnectivityTest"
		testDocID := strfmt.UUID(uuid.New().String())

		defer func() {
			log.Printf("Simple Hello World: Removing Test Class '%s'....", testClassName)
			err := client.Schema().ClassDeleter().WithClassName(testClassName).Do(context.Background())
			if err != nil {
				log.Printf("WARNING: Failed to clean up test class '%s'. Error: %v", testClassName, err)
			} else {
				log.Printf("Hello world test class successfully deleted", testClassName)
			}
		}()

		err = services.InsertSimpleTestDoc(client, testClassName, testDocID, message)
		if err != nil {
			transport.SendServerRes(w, []byte("Simple hello world failed to Insert test doc(insert step)"+err.Error()), http.StatusInternalServerError, err)
			return
		}
		log.Printf("Hello world doc: successfully inserted with ID: %s", testDocID)

		retrievedDoc, err := services.GetSimpleTestDoc(client, testClassName, testDocID)
		if err != nil {
			transport.SendServerRes(w, []byte("Hello world failed to retrieve test doc: "+err.Error()), http.StatusInternalServerError, err)
			return
		}
		log.Printf("Hellow world retrieved doc successfully")

		finalResponse := map[string]interface{}{
			"status":            "SUCCESS",
			"message":           "Weaviate connection is working. document was inserted and retrieved, and test class deleted",
			"retrievedDocument": retrievedDoc.Properties,
		}
		jsonResp, _ := json.Marshal(finalResponse)
		transport.SendServerRes(w, jsonResp, http.StatusOK, nil)
	}
}
...
//
// 	res, err := services.BulkUpsertEventToWeaviate(weaviateClient, createEvents)
// 	if err != nil {
...
// }
//
// // Marqo-PostBatch Handler
// func PostBatchEventsHandler(w http.ResponseWriter, r *http.Request) http.HandlerFunc {
// 	weaviateService := services.NewMarqoService()
// 	handler := NewMarqoHandler(weaviateService)
// 	return func(w http.ResponseWriter, r *http.Request) {
