...
func TestEnvironmentVariableHandling(t *testing.T) {
	...
	defer os.Setenv("APEX_URL", originalApexURL)

	// Test with different APEX_URL formats
	testCases := []struct {
		apexURL        string
	...
	for _, tc := range testCases {
		...
		app := &App{
			...
			AuthConfig: &AuthConfig{
				AuthDomain:     "test.zitadel.cloud",
				AllowedDomains: []string{strings.Replace(tc.apexURL, "https://", "", 1), strings.Replace(tc.apexURL, "https://", "*.", 1), "https://seshu.test.com"},
				CookieDomain:   strings.Replace(strings.Replace(tc.apexURL, "https://", "", 1), "http://", "", 1),
			},
		}
